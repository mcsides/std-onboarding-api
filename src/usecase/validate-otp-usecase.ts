...
export class ValidateOtpUsecase {
  ...
  async exe(
    ...
    this.logger.log(`Validating OTP for email: ${email} with onboardingId: ${onboardingId}`);
    const onboardingFound = await this.onboardingRepository.findBy({'email': email, 'onboardingId': onboardingId});
    if (!onboardingFound) {
      this.logger.error(`No onboarding found for email: ${email} with onboardingId: ${onboardingId}`);
      throw new Error('Onboarding not found');
    } else {
      this.logger.log(`Onboarding found for email: ${email} with onboardingId: ${onboardingId}`);
      if(onboardingFound.getStatus() !== OnboardingStatus.INITIATED) {
        ...
        this.logger.log(`Validating OTP: ${otp} for email: ${email}, is valid: ${isValidOtp}`);
        return isValidOtp;
      }
    }
  }
