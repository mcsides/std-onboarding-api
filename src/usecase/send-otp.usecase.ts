...
export class SendOtpUsecase {
  ...
  async exe( email: string, onboardingId: string): Promise<string> {
    ...
    if (!onboardingFound) {
      this.logger.error(`Onboarding not found for email: ${email} and onboardingId: ${onboardingId}`);
      throw new Error(`Onboarding not found for email: ${email} and onboardingId: ${onboardingId}`);
    } else {
      this.logger.log(`Onboarding found for email: ${email} and onboardingId: ${onboardingId}`);
      if (onboardingFound.getStatus() !== 'INITIATED') {
        this.logger.error(`Onboarding not available. email: ${email}, onboardingId: ${onboardingId}, status: ${onboardingFound.getStatus()}`);
        throw new Error(`Onboarding not available. email: ${email} onboardingId: ${onboardingId}, status: ${onboardingFound.getStatus()}`);
      } else {    
